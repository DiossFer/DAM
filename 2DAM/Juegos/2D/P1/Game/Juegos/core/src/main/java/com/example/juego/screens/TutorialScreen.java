...
public class TutorialScreen implements Screen {
    ...
    private void createUI() {
        ...
        Drawable d = new TextureRegionDrawable(new TextureRegion(Textures.tutorial1));
        
        ImageButton helpImagens = new ImageButton(d);
        helpImagens.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                 Drawable dNew = new TextureRegionDrawable(new TextureRegion(Textures.getNextTutorial()));
                 helpImagens.getStyle().imageUp=dNew;
            }
        });
        
        table.row();
        table.add(helpImagens).padBottom(10);
