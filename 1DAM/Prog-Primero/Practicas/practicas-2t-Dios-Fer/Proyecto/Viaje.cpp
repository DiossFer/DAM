...
void mostrar_viaje_fila_nombres_no_structs (){
    ...
    cout<<"Numero_asientos_ofertados";
    cout <<"\t\t";
    cout << endl <<RESTORE;
}
void mostrar_viaje_fila (const Viaje &v){
    ...
    cout <<endl<<BOLDYELLOW<< "Coche del viaje: "<<RESTORE<<endl;
    mostrar_leyenda_coche();
    get_coche_que_viaja (v, aux_c);
    mostrar_coche_fila (aux_c);
    cout <<endl;

    cout << endl<<BOLDYELLOW<<"Trayecto del viaje: "<<RESTORE<<endl;
    mostrar_titulos_trayecto();
    get_trayecto_que_se_realiza (v, aux_tr);
    mostrar_trayecto_fila (aux_tr);
    cout <<endl;
...
void mostrar_vector_viajes (const Viaje v[], const int util_v){
    for (int i=0; i<util_v; i++){
        mostrar_viaje_fila_nombres_no_structs ();
        mostrar_viaje_fila (v[i]);
    }
}
...
void anadir_viaje_a_vector_viajes(const Viaje &v_aux, Viaje viajes_del_sistema[], int &util_viajes_sistema){
    viajes_del_sistema[util_viajes_sistema]=v_aux;
    util_viajes_sistema++;
}
